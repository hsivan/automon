...
if __name__ == "__main__":
    ...
    try:
        test_folder = start_test("distributed_inner_product", logging_level=logging.INFO)
        ...
        if args.node_idx == -1:
            coordinator = AutomonCoordinator(conf["num_nodes"], func_inner_product, slack_type=SlackType(conf["slack_type"]), sync_type=SyncType(conf["sync_type"]),
        ...
        if args.node_idx >= 0:
            ...
            node = AutomonNode(args.node_idx, func_inner_product, domain=conf["domain"], d=conf["d"])
