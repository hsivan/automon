...
def visualize_entropy():
    ...
    node3 = NodeAutoMon(idx=node_idx, x0_len=k, max_f_val=func_entropy(np.ones(k, dtype=np.float) / k), min_f_val=0.0, domain=(0 + epsilon, 1 - epsilon), func_to_monitor=func_entropy)
    ...
    node2 = NodeAutoMon(idx=node_idx, x0_len=k, max_f_val=func_entropy(np.ones(k, dtype=np.float) / k), min_f_val=0.0, func_to_monitor=func_entropy)
...
def visualize_variance():
    ...
    node = NodeAutoMon(idx=node_idx, x0_len=2, func_to_monitor=func_variance, min_f_val=0.0)
...
def visualize_rozenbrock():
    ...
    dc_type, extreme_lambda = adcd_helper.adcd_x(x0, None, 0)
    node = NodeAutoMon(idx=node_idx, x0_len=x0.shape[0], func_to_monitor=func_rozenbrock)
    node.sync(x0, slack, 0.08, 3, -1, dc_type, extreme_lambda)
    # Fill sliding window
...
def visualize_inner_product():
    ...
    dc_type, extreme_lambda = adcd_helper.adcd_x(x0, None, 0)
    node = NodeAutoMon(idx=node_idx, x0_len=x0.shape[0], func_to_monitor=func_inner_product)
    node.sync(x0, slack, 0.08, 3, -1, dc_type, extreme_lambda)
    # Fill sliding window
    node_stream = NodeStreamAverage(2, sliding_window_size, x0.shape[0], x0.shape[0])
    for i in range(sliding_window_size):
