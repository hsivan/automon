...
from gm_monitoring.node_entropy_gm import NodeEntropyGM
from gm_monitoring.node_variance_gm import NodeVarianceGM
...
def entropy_gm_draw_constraints(node):
    ...
    def entropy_gm_draw_constraints_3d():
        ...
        f_max = func_entropy(np.ones(node.k) / node.k)
    ...
    def entropy_gm_draw_constraints_2d():
        ...
        ax.plot(node.x[0], func_entropy(node.x), 'bo', label='$p$ local', markersize=4, color='yellow',
...
def visualize_entropy():
    ...
    node3 = NodeEntropyGM(idx=1, local_vec_len=3)
    ...
    node2 = NodeEntropyGM(idx=1, local_vec_len=2)
...
def visualize_variance():
    ...
    node = NodeVarianceGM(idx=1)
    node.sync(np.array([0.5, 2]), slack, 0.08, 3)
    # Fill sliding window
    for i in range(sliding_window_size - 1):
