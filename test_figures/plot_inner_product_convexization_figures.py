...
if __name__ == "__main__":
    ...
    slack = slack = np.zeros(2)  # No slack
    x0 = np.array([0.1, 0.1])
    x = np.array([0.65, 0.65])

    dc_type, signed_H, g_func_grad, h_func_grad = adcd_helper.adcd_e(x0)
    node = NodeInnerProductAutoMon(idx=1, sliding_window_size=1, local_vec_len=2)
    node.sync(x0, slack, func_inner_product(x0)-0.5, func_inner_product(x0)+0.5, None, dc_type, signed_H, g_func_grad(x0), h_func_grad(x0))
    ...
    node = NodeInnerProductAutoMon(idx=1, sliding_window_size=1, local_vec_len=2)
    node.sync(x0, slack, func_inner_product(x0) - 0.5, func_inner_product(x0) + 0.5, None, dc_type, extreme_lambda, g_func_grad(x0), h_func_grad(x0))
    b_inside_safe_zone = node.set_new_data_point(x)
