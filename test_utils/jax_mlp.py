...
def draw_f_approx_contour_and_node_trail(net_apply, network_params, nodes_data, test_folder=None, low=-2, high=2):
    ...
    node1_start = jnp.mean(node1_trail[20:40], axis=0)
    node1_end = jnp.mean(node1_trail[-20:], axis=0)
    node6_start = jnp.mean(node6_trail[20:40], axis=0)
...
if __name__ == "__main__":
    train_net(test_folder="./", x_dim=20, num_train_iter=100000, step_size=1e-4)
