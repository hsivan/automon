...
import numpy as np
from automon.data_generator import DataGeneratorQuadratic, DataGeneratorKldAirQuality, DataGeneratorDnnIntrusionDetection, DataGeneratorInnerProduct
from automon.functions_to_monitor import set_H, set_net_params
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.type == "kld":
            ...
            conf = read_config_file(data_folder)
            data_generator = DataGeneratorKldAirQuality(num_iterations=conf["num_iterations"], num_nodes=conf["num_nodes"], d=conf["d"], test_folder=test_folder, num_iterations_for_tuning=conf["num_iterations_for_tuning"])
        if args.type == "dnn":
        ...
        if args.node_idx >= 0:
            run_centralization_node(args.host, args.port, args.node_idx, data_generator, conf["num_nodes"], conf["sliding_window_size"])
