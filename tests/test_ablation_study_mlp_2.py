...
import logging
from automon.jax_mlp import train_net, draw_f_approx_contour_and_node_trail
...
if __name__ == "__main__":
    try:
        ...
        data_generator = DataGeneratorMlp(num_iterations=conf["num_iterations"], num_nodes=conf["num_nodes"], data_file_name="data_file.txt", test_folder=data_folder, d=conf["d"], num_iterations_for_tuning=conf["num_iterations_for_tuning"])
        '''
        ...
        conf = get_config(num_nodes=10, num_iterations=1020, sliding_window_size=20, d=2, error_bound=0.15,
                          ...
                          domain_type=DomainType.Relative.value, neighborhood_size=0.5, num_iterations_for_tuning=200)
        write_config_to_file(test_folder, conf)
        net_params, net_apply = train_net(test_folder, num_train_iter=20000, step_size=1e-4)
        set_net_params(net_params, net_apply)
        data_generator = DataGeneratorMlp(num_iterations=conf["num_iterations"], num_nodes=conf["num_nodes"], d=conf["d"], test_folder=test_folder, num_iterations_for_tuning=conf["num_iterations_for_tuning"])
