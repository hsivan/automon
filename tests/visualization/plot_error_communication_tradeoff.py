...
from automon_utils.stats_analysis_utils import get_period_approximation_error
from automon_utils.test_utils import read_config_file, write_config_to_file
import os
import numpy as np
...
def plot_max_error_vs_communication(parent_test_folder, func_name):
    ...
    num_messages_arr_automon, max_error_arr_automon, error_bound_arr, avg_error_arr_automon, p99_error_arr_automon, centralization_num_messages = max_error_vs_communication(parent_test_folder, func_name, ax, b_use_aggregated_data=False)
    ax.legend()
    ...
    # Figure of % error (relative error with respect to the error bound)
    fig, ax = plt.subplots(figsize=get_figsize(hf=0.4))
    ax.set_ylabel('percent of bound')
    ax.set_xlabel("#messages")
    # Vertical line indication centralization
    ax.axvline(x=centralization_num_messages, color="black", linestyle=":", label="Centralization", linewidth=0.8)
    ax.plot(num_messages_arr_automon, (np.array(max_error_arr_automon) / np.array(error_bound_arr)) * 100, label='max', marker="+", markersize=3, color="C9", linewidth=0.8, linestyle='--')
    ax.plot(num_messages_arr_automon, (np.array(p99_error_arr_automon) / np.array(error_bound_arr)) * 100, label='p99', marker="x", markersize=3, color="tab:blue", linewidth=0.8)
    # horizontal line indicating 100%
    ax.plot([0, np.max(num_messages_arr_automon)], [100, 100], "k--", linewidth=0.8)
    ax.xaxis.set_major_formatter(tick.FuncFormatter(reformat_large_tick_values))
    ax.set_xlim(left=0)
    ...
    ax.legend()
    plt.subplots_adjust(top=0.99, bottom=0.26, left=0.16, right=0.95, hspace=0.1, wspace=0.1)
    fig.savefig(parent_test_folder + "/percent_error.pdf")
    plt.close(fig)
